...
SDL_AppResult SDL_AppInit(void** appstate, int /*argc*/, char** /*argv*/) {
    ...
    *appstate = app;
    ...
    if (!pairing) {
        SDL_Log("Pairing was cancelled or the manager code could not be stored.");
        return SDL_APP_FAILURE;
    }
    ...
    for (int pairAttempt = 0; pairAttempt < 4 && !ok; ++pairAttempt) {
        ...
        if (!sock) {
            ...
            showPairingError("Could not reach the worker host.\nMake sure "
                             "OpenShareCompanion is running on it, both machines are on "
        ...
        for (int authAttempt = 0; authAttempt < 3; ++authAttempt) {
            ...
            if (!authResult || authResult->type != MsgType::AuthResult) {
                ...
                showPairingError("The worker host accepted the connection but never "
                                 "replied in time.\nStop OpenShareCompanion on the "
                                 "worker completely, launch it again, then "
        ...
        if (!ok && pairAttempt == 0) {
            showPairingError("Pairing was rejected by the worker — the manager "
                             "code does not match.\nOpen OpenShareCompanion on the "
                             "worker to see its current code, then enter it here.");
        }
    }

    if (!ok || !sock) {
        SDL_Log("Authentication failed (manager code mismatch).");
        showPairingError("Could not pair with the worker host after several tries.");
        return SDL_APP_FAILURE;
    }
